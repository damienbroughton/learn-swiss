...
export async function getStory(uid, reference) {
    try {
        ...
        if (!story)
            throw new Error(`Story with reference ${reference} was not found.`);
        const flashcards = story?.flashcards;
...
export async function addFlashCardsToStory(uid, id, flashcardIds) {
    try {
        ...
        const updatedStory = await db.collection('stories').findOneAndUpdate({ _id: new ObjectId(id) }, {
