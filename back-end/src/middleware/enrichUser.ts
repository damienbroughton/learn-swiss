 ...
 */
export async function enrichUser(req: EnrichedRequest, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
        ...
        if (req.user?.uid) {
            ...
            const dbUser = await db.collection<UserDocument>('users').findOne({ uid: req.user.uid });
